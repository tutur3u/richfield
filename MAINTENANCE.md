...
| 17 | Upstream delivery cache is unverified | `lib/richfield-delivery.ts` fetch to `/external-projects/delivery` | Publishing now expires our own cache immediately (tag + warm), but we have never checked whether the platform caches the delivery payload itself. If a publish still lags, run `curl -sS -o /dev/null -D - "$TUTURUUU_API_BASE_URL/workspaces/$WORKSPACE_ID/external-projects/delivery"` and read `age`, `cache-control`, `x-vercel-cache` / `cf-cache-status`. Send it as a GET (as above) rather than `-I`: a HEAD response is a different cache entry, so its headers do not have to describe what a real page render receives. A cached upstream body means the remaining delay is platform-side and cannot be fixed here. |
