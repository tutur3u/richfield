...
export function MembersPanel({ membersHref: _membersHref }: { membersHref?: string }) {
  ...
  const [inviteRoleId, setInviteRoleId] = useState("default");
  ...
  async function invite() {
    ...
    if (await mutate("POST", { emails: parsed, roleId: inviteRoleId === "default" ? null : inviteRoleId }, t("inviteSuccess"))) { setEmails(""); setInviteRoleId("default"); setInviteOpen(false); }
  }
  ...
  async function updateRole(role: RichfieldAdminRole, enabled: boolean) {
    if (!accessMember || accessMember.isCurrentUser) return;
    if (accessMember.status === "Invited") {
      if (!accessMember.email) return;
      const roleIds = enabled
        ? [...new Set([...accessMember.roles.map((item) => item.id), role.id])]
        : accessMember.roles.filter((item) => item.id !== role.id).map((item) => item.id);
      await mutate("PATCH", { email: accessMember.email, roleIds }, t("invitationAccessUpdated"));
      return;
    }
    await mutate("PATCH", { enabled, roleId: role.id, userId: accessMember.id }, t(enabled ? "accessLevelAdded" : "accessLevelRemoved"));
  }
  async function selectWorkspaceDefault() {
    if (!accessMember?.email || accessMember.status !== "Invited") return;
    await mutate("PATCH", { email: accessMember.email, roleIds: [] }, t("invitationAccessUpdated"));
  }
  ...
  return <section className="grid min-w-0 gap-6">
    ...
    <MemberGroup accessLevelsLabel={t("accessLevels")} caption={t("activeCaption")} canManage={Boolean(context?.canManageMembers)} canManageRoles={Boolean(context?.canManageRoles)} defaultAccessLabel={t("workspaceDefault")} members={active} onAccess={setAccessMemberId} onRemove={setRemoveTarget} removeLabel={t("removeMemberLabel")} selfRemovalLabel={t("selfRemovalLabel")} title={t("activeTitle")} />
    <MemberGroup accessLevelsLabel={t("accessLevels")} caption={t("invitedCaption")} canManage={Boolean(context?.canManageMembers)} canManageRoles={Boolean(context?.canManageRoles)} defaultAccessLabel={t("workspaceDefault")} members={invited} onAccess={setAccessMemberId} onRemove={setRemoveTarget} removeLabel={t("removeMemberLabel")} selfRemovalLabel={t("selfRemovalLabel")} title={t("invitedTitle")} />

    <Dialog onOpenChange={setInviteOpen} open={inviteOpen}><DialogContent><DialogHeader><DialogTitle>{t("inviteTitle")}</DialogTitle><DialogDescription>{t("inviteDescription")}</DialogDescription></DialogHeader><div className="grid gap-4"><label className="grid gap-2"><span className="text-xs font-bold uppercase tracking-wider text-admin-ink-soft">{t("emails")}</span><textarea autoFocus className="min-h-32 resize-y rounded-xl border border-admin-rule bg-admin-surface px-3 py-3 text-sm text-admin-ink outline-none placeholder:text-admin-ink-soft/60 focus:border-admin-gold focus:ring-3 focus:ring-admin-gold/10" disabled={busy} onChange={(event) => setEmails(event.currentTarget.value)} placeholder={t("invitePlaceholder")} value={emails} /></label><label className="grid gap-2"><span className="text-xs font-bold uppercase tracking-wider text-admin-ink-soft">{t("defaultAccess")}</span><select className="min-h-11 rounded-xl border border-admin-rule bg-admin-surface px-3 text-sm text-admin-ink outline-none focus:border-admin-gold" disabled={busy || !context?.canManageRoles} onChange={(event) => setInviteRoleId(event.currentTarget.value)} value={inviteRoleId}><option value="default">{t("workspaceDefault")}</option>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select><span className="text-xs leading-5 text-admin-ink-soft">{t("defaultAccessHint")}</span></label></div><DialogFooter><DialogClose disabled={busy}>{t("cancel")}</DialogClose><Button className="bg-admin-navy text-white hover:bg-admin-copper" disabled={busy} onClick={() => void invite()}>{busy ? t("saving") : t("invite")}</Button></DialogFooter></DialogContent></Dialog>
    <Dialog onOpenChange={(open) => !open && setAccessMemberId(null)} open={Boolean(accessMember)}><DialogContent><DialogHeader><DialogTitle>{t("accessLevels")}</DialogTitle><DialogDescription>{accessMember?.status === "Invited" ? t("pendingAccessDescription", { email: accessMember?.email ?? accessMember?.name ?? "" }) : accessMember?.email ?? accessMember?.name}</DialogDescription></DialogHeader><div className="grid gap-2">{accessMember?.status === "Invited" ? <button className={`flex min-h-12 items-center justify-between gap-4 rounded-xl border px-4 py-3 text-left text-sm font-medium transition-colors ${accessMember.roles.length === 0 ? "border-admin-gold bg-admin-gold/10" : "border-admin-rule bg-admin-surface hover:border-admin-gold"}`} disabled={busy} onClick={() => void selectWorkspaceDefault()} type="button"><span><span className="block">{t("workspaceDefault")}</span><span className="mt-0.5 block text-xs font-normal text-admin-ink-soft">{t("workspaceDefaultHint")}</span></span><input aria-hidden checked={accessMember.roles.length === 0} className="size-4 accent-admin-navy" readOnly type="radio" /></button> : null}{roles.map((role) => { const checked = accessMember?.roles.some((assigned) => assigned.id === role.id) ?? false; return <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-admin-rule bg-admin-surface px-4 py-3 text-sm font-medium hover:border-admin-gold" key={role.id}><span>{role.name}</span><input checked={checked} className="size-4 accent-admin-navy" disabled={busy || Boolean(accessMember?.isCurrentUser)} onChange={(event) => void updateRole(role, event.currentTarget.checked)} type="checkbox" /></label>; })}</div><DialogFooter><DialogClose>{t("done")}</DialogClose></DialogFooter></DialogContent></Dialog>
